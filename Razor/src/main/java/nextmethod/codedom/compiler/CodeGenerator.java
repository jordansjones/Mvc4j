...
public abstract class CodeGenerator implements ICodeGenerator {
    ...
    protected abstract void generateAnnotationDeclarationsStart(@Nonnull
                                                                final CodeAnnotationDeclarationCollection attributes
                                                               );

    protected abstract void generateAnnotationDeclarationsEnd(@Nonnull
                                                              final CodeAnnotationDeclarationCollection attributes
                                                             );
    ...
    public void generateCodeFromMember(@Nonnull final CodeTypeMember member, @Nonnull final PrintWriter writer,
    ...
    protected void generateCompileUnit(@Nonnull final CodeCompileUnit compileUnit) {
        ...
        compileUnit.getPackages()
                   .stream()
                   .filter(codePackage -> Strings.isNullOrEmpty(codePackage.getName()))
                   .forEach(this::generatePackage);
    ...
    protected abstract void generateEntryPointMethod(@Nonnull final CodeEntryPointMethod m,
                                                     @Nonnull final CodeTypeDeclaration d
                                                    );
    ...
    protected abstract void generatePropertySetValueReferenceExpression(@Nonnull
                                                                        final CodePropertySetValueReferenceExpression e
                                                                       );
    ...
    protected void initOutput(@Nonnull final Writer output, @Nullable CodeGeneratorOptions options) {
        ...
        this.output = new IndentingPrintWriter(
                                                  new PrintWriter(checkNotNull(output)), options.getIndentString(),
                                                  options.getNewlineString()
        );
        this.options = options;
    }

    @Override
    public void generateCodeFromCompileUnit(@Nonnull final CodeCompileUnit compileUnit, @Nonnull final Writer output,
                                            @Nonnull final CodeGeneratorOptions options
                                           ) {
    ...
    @Override
    public void generateCodeFromExpression(@Nonnull final CodeExpression expression, @Nonnull final Writer output,
                                           @Nonnull final CodeGeneratorOptions options
                                          ) {
        initOutput(output, options);
    ...
    @Override
    public void generateCodeFromPackage(@Nonnull final CodePackage codePackage, @Nonnull final Writer output,
                                        @Nonnull final CodeGeneratorOptions options
                                       ) {
        initOutput(output, options);
    ...
    @Override
    public void generateCodeFromStatement(@Nonnull final CodeStatement statement, @Nonnull final Writer output,
                                          @Nonnull final CodeGeneratorOptions options
                                         ) {
        initOutput(output, options);
    ...
    @Override
    public void generateCodeFromType(@Nonnull final CodeTypeDeclaration type, @Nonnull final Writer output,
                                     @Nonnull final CodeGeneratorOptions options
                                    ) {
        initOutput(output, options);
        generateType(type);

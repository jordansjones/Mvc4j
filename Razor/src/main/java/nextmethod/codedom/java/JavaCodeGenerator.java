...
class JavaCodeGenerator extends CodeGenerator {
    ...
    private void outputAnnotations(@Nonnull final CodeAnnotationDeclarationCollection annotations, final String prefix,
...
//			}
    ...
    private void outputTypeArguments(final CodeTypeReferenceCollection typeArgs, final StringBuilder sb,
                                     final int count
                                    ) {
        if (count == 0) return;
        if (typeArgs.size() == 0) {

...
public class RazorEditorParser implements IDisposable {
    ...
    public RazorEditorParser(@Nonnull final RazorEngineHost host, @Nonnull final String sourceFileName) {
        ...
        checkArgument(
                         Strings.isNullOrEmpty(sourceFileName) == false,
    ...
    public String getAutoCompleteString() {
        if (lastAutoCompleteSpan != null) {
            final AutoCompleteEditHandler editHandler = typeAs(
                                                                  lastAutoCompleteSpan.getEditHandler(),
...
//				Debug.WriteLine("[RzEd] Document Parse Complete Handler Threw: " + ex.ToString());
    ...
    private static void verifyFlagsAreValid(final EnumSet<PartialParseResult> result) {
        if (Debug.isAssertEnabled()) {
            assert result.contains(PartialParseResult.Accepted) ||
                   result.contains(PartialParseResult.Rejected) : "Partial Parse result does not have either of Accepted or Rejected";
            assert result.contains(PartialParseResult.Rejected) ||
                   !result.contains(PartialParseResult.SpanContextChanged) : "Partial Parse result was Accepted AND had SpanContextChanged";
            assert result.contains(PartialParseResult.Rejected) ||
                   !result.contains(PartialParseResult.AutoCompleteBlock) : "Partial Parse result was Accepted AND had AutoCompleteBlock";
            assert result.contains(PartialParseResult.Accepted) ||
                   !result.contains(PartialParseResult.Provisional) : "Partial Parse result was Rejected AND had Provisional";
        }
    }

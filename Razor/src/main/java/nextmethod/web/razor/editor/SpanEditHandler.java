...
public class SpanEditHandler {
    ...
    public void setAcceptedCharacters(@Nonnull final AcceptedCharacters first,
                                      @Nullable final AcceptedCharacters... rest
                                     ) {
    ...
    protected static boolean isAtEndOfFirstLine(@Nonnull final Span target, @Nonnull final TextChange change) {
        final int endOfFirstLine = indexOfAny(
                                                 target.getContent(),
                                                 new char[]{(char) 0x000d, (char) 0x000a, (char) 0x2028, (char) 0x2029}
                                             );
        return (endOfFirstLine == -1 ||
                (change.getOldPosition() - target.getStart().getAbsoluteIndex()) <= endOfFirstLine);
    }
    ...
    protected static boolean isAtEndOfSpan(@Nonnull final Span target, @Nonnull final TextChange change) {
        return (change.getOldPosition() + change.getOldLength()) ==
               (target.getStart().getAbsoluteIndex() + target.getLength());
    }

...
public class CallbackVisitor extends ParserVisitor {
    ...
    public CallbackVisitor(@Nonnull final IAction1<Span> spanCallback,
                           @Nonnull final IAction1<RazorError> errorCallback
                          ) {
        this(
                spanCallback, errorCallback,
                input -> {
                },
                input -> {
                }
    ...
    public CallbackVisitor(@Nonnull final IAction1<Span> spanCallback,
                           @Nonnull final IAction1<RazorError> errorCallback,
                           @Nonnull final IAction1<BlockType> startBlockCallback,
                           @Nonnull final IAction1<BlockType> endBlockCallback
                          ) {
        this(
                spanCallback, errorCallback, startBlockCallback, endBlockCallback, () -> {
            }
            );
    }
    ...
    public CallbackVisitor(@Nonnull final IAction1<Span> spanCallback,
                           @Nonnull final IAction1<RazorError> errorCallback,
                           @Nonnull final IAction1<BlockType> startBlockCallback,
                           @Nonnull final IAction1<BlockType> endBlockCallback, @Nonnull final IAction completeCallback
                          ) {
    ...
    private static <T> void raiseCallback(final SynchronizationContext syncContext, final T param,
                                          final IAction1<T> callback
                                         ) {
        if (callback != null) {
            if (syncContext != null) {

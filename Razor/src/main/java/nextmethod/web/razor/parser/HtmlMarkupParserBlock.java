...
final class HtmlMarkupParserBlock extends HtmlMarkupParserDelegate {
    ...
    protected void afterTransition() {
        ...
        if (isCurrentSymbol(HtmlSymbolType.Text) && getCurrentSymbol().getContent().length() > 0 &&
            getCurrentSymbol().getContent().charAt(0) == ':') {
            // Split the token
            final KeyValue<HtmlSymbol, HtmlSymbol> split = getLanguage().splitSymbol(
                                                                                        getCurrentSymbol(), 1,
    ...
    protected boolean afterTagStart(@Nonnull final SourceLocation tagStart,
                                    @Nonnull final Deque<KeyValue<HtmlSymbol, SourceLocation>> tags
                                   ) {
        if (!isEndOfFile()) {
        ...
        if (isCurrentSymbol(HtmlSymbolType.Text) && "cdata".equalsIgnoreCase(getCurrentSymbol().getContent())) {
            ...
            if (isCurrentSymbol(HtmlSymbolType.LeftBracket)) {
                return acceptUntilAll(
                                         HtmlSymbolType.RightBracket, HtmlSymbolType.RightBracket,
    ...
    protected boolean endTag(@Nonnull final SourceLocation tagStart,
                             @Nonnull final Deque<KeyValue<HtmlSymbol, SourceLocation>> tags
                            ) {
    ...
    protected final Delegates.IFunc1<HtmlSymbol, Boolean> isTagRecoveryStopPointDelegate = input1 -> input1 != null &&
                                                                                                     isTagRecoveryStopPoint(input1);
    ...
    protected void beforeAttribute() {
        ...
        // Capture whitespace
        final Iterable<HtmlSymbol> whitespace = readWhile(
                                                             symbol -> symbol != null && symbol.isTypeOr(
    ...
    protected void attributePrefix(@Nonnull final Iterable<HtmlSymbol> whitespace,
                                   @Nonnull final Iterable<HtmlSymbol> nameSymbols
                                  ) {
        // First, determine if this is a 'data-' attribute (since those can't use conditional attributes)
        final LocationTagged<String> name = SymbolExtensions.getContent(nameSymbols, getSpan().getStart());
        final boolean attributeCanBeConditional = !Strings.nullToEmpty(name.getValue())
                                                          .toLowerCase()
                                                          .startsWith("data-");
        ...
        if (attributeCanBeConditional) {
            ...
            getContext().getCurrentBlock()
                        .setCodeGenerator(new AttributeBlockCodeGenerator(name.getValue(), prefix, suffix));
        }
        else {
    ...
    protected void attributeValue(@Nonnull final HtmlSymbolType quote) {
        ...
        final Iterable<HtmlSymbol> prefix = readWhile(
                                                         symbol -> symbol != null && (symbol.isTypeOr(
                                                                                                         HtmlSymbolType.WhiteSpace,
                                                                                                         HtmlSymbolType.NewLine
                                                                                                     ))
        ...
        if (at(HtmlSymbolType.Transition)) {
            ...
            getSpan().setCodeGenerator(
                                          LiteralAttributeCodeGenerator.fromValueGenerator(
                                                                                              SymbolExtensions.getContent(
                                                                                                                             prefix,
                                                                                                                             prefixStart
                                                                                                                         ),
            ...
            final Iterable<HtmlSymbol> value = readWhile(
                                                            symbol -> symbol != null && (
                                                                                            symbol.getType() !=
                                                                                            HtmlSymbolType.WhiteSpace
                                                                                            && symbol.getType() !=
    ...
    protected boolean isEndOfAttributeValue(@Nonnull final HtmlSymbolType quote, @Nullable final HtmlSymbol sym) {
        return isEndOfFile()
               ...
               || (
                      ...
                      ? sym.getType() == quote
                      // if quoted, just waite for the quote
    ...
    protected void virtualPath() {
        ...
        acceptUntil(
                       HtmlSymbolType.Transition, HtmlSymbolType.WhiteSpace, HtmlSymbolType.NewLine,
    ...
    protected boolean restOfTag(@Nonnull final KeyValue<HtmlSymbol, SourceLocation> tag,
                                @Nonnull final Deque<KeyValue<HtmlSymbol, SourceLocation>> tags
                               ) {
        tagContent();
    ...
    protected boolean acceptUntilAll(@Nonnull final HtmlSymbolType... endSequence) {
        ...
        if (Debug.isAssertEnabled()) { assert isEndOfFile(); }
        getSpan().getEditHandler().setAcceptedCharacters(AcceptedCharacters.Any);
        return false;
    }

    @SuppressWarnings("ConstantConditions")
    protected boolean removeTag(@Nonnull final Deque<KeyValue<HtmlSymbol, SourceLocation>> tags,
                                @Nonnull final String tagName, @Nonnull final SourceLocation tagStart
                               ) {
        ...
        if (currentTag != null) {
            getContext().onError(
                                    currentTag.getValue(),
                                    RazorResources().parseErrorMissingEndTag(currentTag.getKey().getContent())
                                );
        }
        else {

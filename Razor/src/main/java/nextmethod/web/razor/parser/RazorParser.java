...
public class RazorParser {
    ...
    public Task createParseTask(@Nonnull final TextReader input, @Nonnull final Delegates.IAction1<Span> spanCallback,
                                @Nonnull final Delegates.IAction1<RazorError> errorCallback
                               ) {
    ...
    public Task createParseTask(@Nonnull final TextReader input, @Nonnull final Delegates.IAction1<Span> spanCallback,
                                @Nonnull final Delegates.IAction1<RazorError> errorCallback,
                                @Nonnull final SynchronizationContext context
                               ) {
        return createParseTask(
                                  checkNotNull(input),
                                  new CallbackVisitor(checkNotNull(spanCallback), checkNotNull(errorCallback))
    ...
    public Task createParseTask(@Nonnull final TextReader input, @Nonnull final Delegates.IAction1<Span> spanCallback,
                                @Nonnull final Delegates.IAction1<RazorError> errorCallback,
                                @Nullable final CancellationToken cancelToken
                               ) {
        return createParseTask(
                                  checkNotNull(input),
                                  new CallbackVisitor(checkNotNull(spanCallback), checkNotNull(errorCallback))

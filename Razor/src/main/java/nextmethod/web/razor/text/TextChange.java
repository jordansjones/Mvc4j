...
public class TextChange {
    ...
    public TextChange(final int position, final int oldLength, @Nonnull final ITextBuffer oldBuffer,
                      final int newLength, @Nonnull final ITextBuffer newBuffer
                     ) {
        this(position, oldLength, oldBuffer, position, newLength, newBuffer);
    }

    public TextChange(final int oldPosition, final int oldLength, @Nonnull final ITextBuffer oldBuffer,
                      final int newPosition, final int newLength, @Nonnull final ITextBuffer newBuffer
                     ) {
    ...
    public String getOldText() {
        if (oldText == null && oldBuffer != null) { oldText = getText(oldBuffer, oldPosition, oldLength); }
        return oldText;
    }

    public String getNewText() {
        if (newText == null) { newText = getText(newBuffer, newPosition, newLength); }
        return newText;
    }
    ...
    public boolean equals(Object o) {
        ...
        if (newBuffer != null
            ? !newBuffer.equals(that.newBuffer)
            : that.newBuffer != null) { return false; }
        if (newText != null
            ? !newText.equals(that.newText)
            : that.newText != null) { return false; }
        if (oldBuffer != null
            ? !oldBuffer.equals(that.oldBuffer)
            : that.oldBuffer != null) { return false; }
        if (oldText != null
            ? !oldText.equals(that.oldText)
            : that.oldText != null) { return false; }
    ...
    public TextChange normalize() {
        if (oldBuffer != null && isReplace() && newLength > oldLength && getNewText().startsWith(getOldText()) &&
        ...
        try {
            ...
            if (newLength == 1) {
                ...
                for (int i = 0; i < length; i++) {
                    ...
                    sb.append(c);
                    if (Character.isHighSurrogate(c)) { sb.append((char) buffer.read()); }
                }
                return sb.toString();
            }
        }

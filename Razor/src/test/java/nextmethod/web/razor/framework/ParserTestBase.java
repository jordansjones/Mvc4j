...
public abstract class ParserTestBase {
    ...
    protected abstract ParserBase selectActiveParser(@Nonnull final ParserBase codeParser,
                                                     @Nonnull final ParserBase markupParser
                                                    );

    public ParserContext createParserContext(@Nonnull final ITextDocument input, @Nonnull final ParserBase codeParser,
                                             @Nonnull final ParserBase markupParser
                                            ) {
    ...
    protected void parseBlockTest(@Nonnull final String document, @Nonnull final Block expectedRoot,
                                  final boolean designTimeParser
                                 ) {
    ...
    protected void parseBlockTest(@Nonnull final String document, final Block expectedRoot,
                                  final boolean designTimeParser, final RazorError... expectedErrors
                                 ) {
        runParseTest(
                        document, parser -> () -> {
                if (parser != null) {
            ...
            }, expectedRoot, (expectedErrors == null
                              ? Collections.<RazorError>emptyList()
                              : Lists.newArrayList(expectedErrors)), designTimeParser
                    );
    }
    ...
    protected void singleSpanBlockTest(final String document, final BlockType blockType, final SpanKind spanType,
                                       final AcceptedCharacters... acceptedCharacters
                                      ) {
    ...
    protected void singleSpanBlockTest(final String document, final String spanContent, final BlockType blockType,
                                       final SpanKind spanType
                                      ) {
        singleSpanBlockTest(document, spanContent, blockType, spanType, AcceptedCharacters.Any);
    }

    protected void singleSpanBlockTest(final String document, final String spanContent, final BlockType blockType,
                                       final SpanKind spanType, final AcceptedCharacters... acceptedCharacters
                                      ) {
        final EnumSet<AcceptedCharacters> ac = EnumSet.noneOf(AcceptedCharacters.class);
        if (acceptedCharacters != null && acceptedCharacters.length > 0) {
            Collections.addAll(ac, acceptedCharacters);
    ...
    protected void singleSpanBlockTest(final String document, final String spanContent, final BlockType blockType,
        ...
        parseBlockTest(
                          ...
                          configureAndAddSpanToBlock(
                                                        builder,
                                                        factory().span(
                                                                          spanType, spanContent,
                                                                          spanType == SpanKind.Markup
    ...
    protected void parseDocumentTest(final String document, final Block expectedRoot, final boolean designTimeParser,
        ...
        runParseTest(
                        ...
                        expectedRoot,
                        expectedErrors != null
                        ? Lists.newArrayList(expectedErrors)
                        : Collections.<RazorError>emptyList(),
                        designTimeParser,
                        context -> {
                            assert context != null;
    ...
    protected ParserResults runParse(final String document,
                                     final Delegates.IFunc1<ParserBase, Delegates.IAction> parserActionSelector,
                                     final boolean designTimeParser
                                    ) {
    ...
    protected ParserResults runParse(final String document,
                                     Delegates.IFunc1<ParserBase, Delegates.IAction> parserActionSelector,
                                     final boolean designTimeParser,
                                     Delegates.IFunc1<ParserContext, ParserBase> parserSelector
                                    ) {
        parserSelector = parserSelector != null
                         ? parserSelector
                         : c -> c == null
                                ? null
                                : c.getActiveParser();
    ...
    protected void runParseTest(final String document,
                                final Delegates.IFunc1<ParserBase, Delegates.IAction> parserActionSelector,
                                final Block expectedRoot, final List<RazorError> expectedErrors,
                                final boolean designTimeParser
                               ) {
    ...
    public static void evaluateResults(final ParserResults results, final Block expectedRoot,
                                       final List<RazorError> expectedErrors
                                      ) {
    ...
    private static void evaluateSyntaxTreeNode(final ErrorCollector collector, final SyntaxTreeNode actual,
                                               final SyntaxTreeNode expected
                                              ) {
        if (actual == null) {
    ...
    private static void addMismatchError(final ErrorCollector collector, final SyntaxTreeNode actual,
                                         final SyntaxTreeNode expected
                                        ) {
        collector.addError("%s - FAILED :: Actual: %s", expected, actual);
    }

    private static void addNullActualError(final ErrorCollector collector, final SyntaxTreeNode actual,
                                           final SyntaxTreeNode expected
                                          ) {
        ...
        if (expectedErrors == null || expectedErrors.isEmpty()) {
            assertTrue(
                          String.format(
                                       ...
                                       ), actualErrors.isEmpty()
                      );
        }
        else {
            assertTrue(
                          String.format(
                                           "Expected that %d errors would be raised, but %d errors were.\r\nExpected Errors: \r\n%s\r\nActual Errors: \r\n%s",
                                           expectedErrors.size(),
                                           actualErrors.size(),
    ...
    private void writeNode(int indent, final SyntaxTreeNode node) {
        ...
        if (indent > 0) { content = Strings.repeat(".", indent * 2) + content; }
    ...
    protected Block createSimpleBlockAndSpan(final String spanContent, final BlockType blockType,
                                             final SpanKind spanType
                                            ) {
    ...
    protected Block createSimpleBlockAndSpan(final String spanContent, final BlockType blockType,
                                             final SpanKind spanType, EnumSet<AcceptedCharacters> acceptedCharacters
                                            ) {
        if (acceptedCharacters == null) acceptedCharacters = AcceptedCharacters.Any;

        final SpanConstructor span = factory().span(spanType, spanContent, spanType == SpanKind.Markup)
                                              .accepts(acceptedCharacters);

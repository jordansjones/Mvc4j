...
public class HtmlToCodeSwitchTest extends JavaHtmlMarkupParserTestBase {
	...
	public void parseBlockSwitchesToCodeWhenSwapCharacterEncounteredInAttributeValue() {
		parseBlockTest(
			...
			new MarkupBlock(
				factory().markup("<foo"),
				new MarkupBlock(
					new AttributeBlockCodeGenerator("bar", locationTagged(" bar=\"", 4, 0, 4), locationTagged("\"", 14, 0, 14)),
					factory().markup(" bar=\"").with(SpanCodeGenerator.Null),
					new MarkupBlock(
						new DynamicAttributeBlockCodeGenerator(locationTagged(10, 0), 10, 0, 10),
						new ExpressionBlock(
							factory().codeTransition(),
							factory().code("baz")
								.asImplicitExpression(JavaCodeParser.DefaultKeywords)
	...
	public void parseBlockParsesCodeWithinSingleLineMarkup() {
		parseBlockTest(
			...
			new MarkupBlock(
				...
				factory().markup("<li>Foo ").with(new SingleLineMarkupEditHandler(JavaLanguageCharacteristics.Instance.createTokenizeStringDelegate())),
				new ExpressionBlock(
					factory().codeTransition(),
					factory().code("Bar")
						.asImplicitExpression(JavaCodeParser.DefaultKeywords)
	...
	public void parseBlockDoesNotSwitchToCodeOnEmailAddressInText() {
		singleSpanBlockTest("<foo>jordansjones@gmail.com</foo>", BlockType.Markup, SpanKind.Markup, AcceptedCharacters.None);
	}
	...
	public void parseBlockDoesNotSwitchToCodeOnEmailAddressInAttribute() {
		parseBlockTest(
			...
			new MarkupBlock(
				factory().markup("<a"),
				new MarkupBlock(
					new AttributeBlockCodeGenerator("href", locationTagged(" href=\"", 2, 0, 2), locationTagged("\"", 38, 0, 38)),
					...
					factory().markup("mailto:jordansjones@gmail.com")
						.with(LiteralAttributeCodeGenerator.fromValue(
							locationTagged(9, 0),
							locationTagged("mailto:jordansjones@gmail.com", 9, 0, 9)
						)),
					factory().markup("\"").with(SpanCodeGenerator.Null)
				),
		...
		parseDocumentTest(
			...
			new MarkupBlock(
				...
				new SectionBlock(
					...
					factory().codeTransition(),
					factory().metaCode("section foo {").autoCompleteWith(null, true),
					new MarkupBlock(
						factory().markup("\r\n    <ul>\r\n"),
						new StatementBlock(
							factory().code("        ").asStatement(),

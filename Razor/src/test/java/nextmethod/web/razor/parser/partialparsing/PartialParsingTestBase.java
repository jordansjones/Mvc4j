...
public abstract class PartialParsingTestBase<TLanguage extends RazorCodeLanguage> {
    ...
    protected void runPartialParseTest(final TextChange change, final Block newTreeRoot,
                                       final PartialParseResult... additionalFlags
                                      ) {
    ...
    protected void runPartialParseTest(final TextChange change, final Block newTreeRoot,
                                       final EnumSet<PartialParseResult> additionalFlags
                                      ) {
    ...
    protected void runTypeKeywordTest(final String keyword) {
        ...
        runFullReparseTest(
                              ...
                              PartialParseResult.setOf(PartialParseResult.SpanContextChanged)
                          );
    }
    ...
    protected class TestParserManager {
        ...
        public TestParserManager(final RazorEditorParser parser) {
            ...
            parser.setDocumentParseCompleteHandler(
                                                      new IEventHandler<DocumentParseCompleteEventArgs>() {

                                                          @Override
                                                          public void handleEvent(@Nonnull final Object sender, @Nonnull
                                                          final DocumentParseCompleteEventArgs e
                                                                                 ) {
                                                              parserMonitor.enter();
                                                              try {
        ...
        public void initializeWithDocument(final ITextBuffer startDocument) {
            checkForStructureChangesAndWait(
                                               new TextChange(
                                                                 0, 0, new StringTextBuffer(Strings.Empty),
                                                                 startDocument.getLength(), startDocument
                                               )
                                           );
        }
        ...
        public void waitForParse() {
            ...
            MiscUtils.DoWithTimeoutIfNotDebugging(
                                                     timeoutInMillis -> {
                                                         assert timeoutInMillis != null;
                                                         boolean wasSignaled = parserMonitor.enterWhenUninterruptibly(
                                                                                                                         parseCompleteGuard,
                                                                                                                         timeoutInMillis,

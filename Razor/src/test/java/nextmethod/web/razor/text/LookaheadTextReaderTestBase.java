...
public abstract class LookaheadTextReaderTestBase {
    ...
    protected void runSourceLocationTest(@Nonnull final String input, @Nonnull final SourceLocation expected,
                                         final int checkAt
                                        ) {
        runSourceLocationTest(
                                 input, expected, input1 -> {
    ...
    protected void runSourceLocationTest(@Nonnull final String input, @Nonnull final SourceLocation expected,
                                         @Nonnull final Function<LookaheadTextReader, LookaheadTextReader> readerAction
                                        ) {
    ...
    protected Func<StringBuilder, LookaheadTextReader> captureSourceLocation(@Nonnull
                                                                             final Function<SourceLocation, SourceLocation> capture
                                                                            ) {
    ...
    @SafeVarargs
    protected final void runLookaheadTest(final String input, final String expected,
                                          final Func<StringBuilder, LookaheadTextReader>... readerCommands
                                         ) {
        final StringBuilder sb = new StringBuilder();
        try (LookaheadTextReader reader = createReader(input)) {
            runAll(readerCommands, sb, reader);
        }
        if (expected != null) { assertEquals(expected, sb.toString()); }
    }

    protected void runReadUntilTest(@Nonnull final Function<LookaheadTextReader, String> readMethod,
                                    final int expectedRaw, final int expectedChar, final int expectedLine
                                   ) {
    ...
    protected void runBufferReadTest(@Nonnull
                                     final Func4<LookaheadTextReader, char[], Integer, Integer, Integer> readMethod
                                    ) {
        final LookaheadTextReader reader = createReader("abcdefg");
    ...
    private static void runAll(final Func<StringBuilder, LookaheadTextReader>[] readerCommands,
                               @Nonnull final StringBuilder builder, @Nonnull final LookaheadTextReader reader
                              ) {
    ...
    private void runPeekOrReadTest(final String input, final int offset, final boolean isRead) {
        try (final LookaheadTextReader reader = createReader(input)) {
            ...
            final int actual = isRead
                               ? reader.read()
                               : reader.peek();

            assertReaderValueCorrect(actual, input, offset, "Peek");
            if (isRead) { assertReaderValueCorrect(reader.peek(), input, offset + 1, "Read"); }
            else { assertEquals(actual, reader.peek()); }
        }
    }
    ...
    private void assertReaderValueCorrect(final int actual, final String input, final int expectedOffset,
                                          final String methodName
                                         ) {
        if (expectedOffset < input.length()) { assertEquals(input.charAt(expectedOffset), (char) actual); }
        else { assertEquals(-1, actual); }
    }

    protected interface Func4<POne, PTwo, PThree, PFour, RType> {

        public RType apply(@Nonnull final POne inputOne, @Nonnull final PTwo inputTwo, @Nonnull final PThree inputThree,
                           @Nonnull final PFour inputFour
                          );
    }

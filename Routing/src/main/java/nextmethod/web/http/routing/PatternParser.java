...
import static nextmethod.web.http.routing.Helpers.rvdHas;
...
class PatternParser {
	...
	HttpRouteValueDictionary addDefaults(final HttpRouteValueDictionary dict, final HttpRouteValueDictionary defaults) {
	...
	boolean matchSegment(int segIdex, int argsCount, String[] argSegs, List<PatternToken> tokens, HttpRouteValueDictionary ret) {
	...
	public HttpRouteValueDictionary match(final String path, final HttpRouteValueDictionary defaults) {
		final HttpRouteValueDictionary ret = new HttpRouteValueDictionary();
	...
	public String buildUrl(final Route route, final RequestContext requestContext, HttpRouteValueDictionary userValues) {
		if (requestContext == null)
			return null;

		final RouteData routeData = requestContext.getRouteData();
		HttpRouteValueDictionary defaultValues = route != null ? route.getDefaults() : null;
		HttpRouteValueDictionary ambientValues = routeData.getValues();
		...
		final HttpRouteValueDictionary constraints = route != null ? route.getConstraints() : null;
		if (constraints != null && !constraints.isEmpty()) {
			...
			for (KeyValuePair<String, Object> entry : constraints) {
				if (!Route.processConstraintInternal(httpContext, route, entry.getValue(), entry.getKey(), userValues, HttpRouteDirection.UriGeneration, requestContext, invalidConstraint))

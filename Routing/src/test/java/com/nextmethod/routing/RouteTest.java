...
public class RouteTest extends BaseTest {
	...
	public void testGetRouteDataNoTemplate() {
		final HttpContext httpContext = mockRequest("~/foo/bar", "");
	...
	public void testInvalidConstraint() {
		...
		final HttpContext mockContext = mockRequest("~/x/y", "");
	...
	public void testGetRouteData() {
		final Route r = new Route("{foo}/{bar}", null);
		final HttpContext hc = mockRequest("~/x/y", "");
	...
	public void testGetRouteData2() {
		...
		final HttpContext hc = mockRequest("~/x/bart", "");
		final RouteData rd = r.getRouteData(hc);
	...
	public void testGetRouteData3() {
		final Route r = new Route("{foo}/{bar}", null);
		final HttpContext hc = mockRequest("~/x/y/z", "");
		final RouteData rd = r.getRouteData(hc);
	...
	public void testGetRouteData4() {
		final Route r = new Route("{foo}/{bar}", null);
		final HttpContext hc = mockRequest("~/x", "");
		final RouteData rd = r.getRouteData(hc);
		assertNull(rd); // mismatch
	}
	...
	public void testGetRouteData8() {
		...
		final HttpContext hc = mockRequest("~/a/b/c/d", "");
		final RouteData rd = r.getRouteData(hc);
		assertNotNull("#1", rd);
		assertEquals("#2", r, rd.getRoute());
		assertEquals("#3", 0, rd.getDataTokens().size());
	...
	private static HttpContext mockRequest(final String requestedUrl, final String contextPath) {
		final HttpRequest mockRequest = when(mock(HttpRequest.class).getAppRelativeCurrentExecutionFilePath()).thenReturn(requestedUrl).getMock();
		final ServletContext context = when(mock(ServletContext.class).getContextPath()).thenReturn(contextPath).getMock();
		return new HttpContext(context, mockRequest, mock(HttpServletResponse.class));
	}
